...
#include "stockData.h"
    ...
    void stockData::printStockByYear(int year) {
    cout << this->stockName << endl;
    int month;
    for (int i = 0; i < this->dataSize; ++i) {
        month = this->dataNodeLst[i]->getMonth();
        if(this->dataNodeLst[i]->isInYear(year)) {
            if(month >= 10) {
                cout << month << ": [ " << this->dataNodeLst[i]->toString();
                cout << "6. reported EPS: " << getEPS(year, month) << " ]" << endl;
            }
            else {
                cout << month << ":  [ " << this->dataNodeLst[i]->toString();
                cout << "6. reported EPS: " << getEPS(year, month) << " ]" << endl;
    ...
    void stockData::compareByPriceToEarnings() {
        int spaceCounter;
        float highAverage;
        ...
        cout << (int)(highAverage / epsAverage) << endl;
    }
